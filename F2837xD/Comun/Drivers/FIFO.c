...
void Init_FIFO (FIFO *fifo, uint16_t NumWords, uint16_t NumMsg)
{
  ...
  fifo->Datos_FIFO      = (uint16_t *)calloc((fifo->T_WORDS+1), sizeof(uint16_t));
...
void Encolar_FIFO (FIFO *fifo)
{
  ...
  switch (fifo->Estado_PILA)
    ...
    if (fifo->Flags.bit.recibir > 0 ){
        ...
        for (fifo->Flags.bit.array = 0; fifo->Flags.bit.array <= (fifo->N_WORDS-1); fifo->Flags.bit.array++){
            memcpy((void *)(fifo->Datos_FIFO + fifo->Ptr_escribir), (void *)(fifo->New_Datos + fifo->Flags.bit.array), 1);
...
//        if (fifo->Msg_recibido == fifo->Msg_leido){
...
void Desencolar_FIFO  (FIFO *fifo)
{
    ...
    if (fifo->Flags.bit.leer > 0){
        ...
        for (fifo->Flags.bit.array = 0; fifo->Flags.bit.array <= (fifo->N_WORDS-1); fifo->Flags.bit.array++){
            memcpy((void *)(fifo->Datos_Recibidos + fifo->Flags.bit.array), (void *)(fifo->Datos_FIFO + fifo->Ptr_leer), 1);
            fifo->Ptr_leer ++;
        }
        fifo->Msg_pendientes --;
